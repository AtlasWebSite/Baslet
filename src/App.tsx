...
function AuthenticatedApp({ user }: { user: NonNullable<ReturnType<typeof useAuth>['user']> }) {
  ...
  const cancelPlan = async () => { if (!window.confirm('Encerrar o Premium ao fim do período pago? Você continua com acesso até essa data.')) return; await billing.cancel(); notify('info', 'Seu acesso continua até o fim do período pago.'); };
