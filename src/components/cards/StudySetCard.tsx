...
export function StudySetCard({ studySet, onStudy, isPremium = true }: { studySet: StudySet; onStudy: (studySet: StudySet) => void; isPremium?: boolean }) {
  ...
  return (
    <article className="set-card" data-tour="study-set-card" style={{ '--set-color': studySet.color } as CSSProperties}>
      ...
      <div className="set-card__footer">
        ...
        <button data-tour="study-set-study-button" className={!isPremium ? 'premium-action-inline' : undefined} onClick={() => onStudy(studySet)}>
          {isPremium ? 'Estudar' : 'Premium'} {isPremium ? <Play size={15} fill="currentColor" /> : <Crown size={14} />}
        </button>
      </div>
    </article>
  );
