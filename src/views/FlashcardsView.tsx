...
export function FlashcardsView({ studySet, startCardId, studySets, isPremium, onRequirePremium, onChange, onUpdate, onRate, onBack }: { studySet?: StudySet; startCardId?: string; studySets: StudySet[]; isPremium: boolean; onRequirePremium: (message?: string) => void; onChange: (studySet: StudySet) => void; onUpdate: (studySet: StudySet) => void; onRate: (studySet: StudySet, flashcardId: string, mastery: 1|2|3) => Promise<void>; onBack: () => void }) {
  ...
  return (
    <div className="view flashcards-view">
      ...
      <div className="session-progress"><span style={{ width: `${((index + 1) / studySet.cards.length) * 100}%` }} /></div>
      <button className={`flashcard ${flipped ? 'is-flipped' : ''}`} onClick={() => setFlipped((current) => !current)} aria-label="Virar flashcard">
      ...
      <div className="card-navigation"><button onClick={() => go(-1)} aria-label="Card anterior"><ChevronLeft size={22} /></button><span>{feedback ?? (flipped ? 'Como foi lembrar?' : 'Pense na resposta antes de virar')}</span><button onClick={() => go(1)} aria-label="Próximo card"><ChevronRight size={22} /></button></div>
