...
import { touchProfileSession, upsertProfileFromSession } from './db.js';
import { recordApplicationError } from './activity.js';
import { getAppUrl, json, methodNotAllowed, readJsonBody, requireEnvironment } from './http.js';
import {
...
async function handleSubscriptionCancel(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    ...
    json(response, 400, { error: 'Não foi possível cancelar a assinatura agora.' });
  }
}
...
async function handleMercadoPagoWebhook(request: VercelRequest, response: VercelResponse) {
  if (getMethod(request) !== 'POST') {
    methodNotAllowed(response);
    return;
  }
...
export async function handlePublicRequest(
  ...
  forcedResource?: string,
) {
  const routeSegments = getRouteSegments(request);
  const [resource, action] = forcedResource ? [forcedResource, ...routeSegments] : routeSegments;
  ...
  if (resource === 'mercado-pago' && action === 'webhook') return handleMercadoPagoWebhook(request, response);
